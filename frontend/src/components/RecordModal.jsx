...
export default function RecordModal({ record, onClose, onSave, defaultFormType }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // ── Validation ──────────────────────────────────────────────
      // Pay Level validation for Honorarium, TA/DA, Salary
      ...
      // Nature of Programme & Title — required and min 4 chars for all non-refund forms
      if (selectedFormType !== 'refund') {
        const natureVal = (formData.nature_of_programme || formData.programme_nature || formData.natureOfProgramme || "").trim();
        ...
        const titleVal = (formData.title_of_programme || formData.programme_title || formData.titleOfProgramme || "").trim();
      ...
      // Journey fields validation for TA/DA
      if (selectedFormType === 'tada') {
